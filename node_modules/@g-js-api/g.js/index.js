...
class Context {
  ...
  static addObject(objectToAdd) {
    ...
    Context.findByName(Context.current).objects.push(addCallback(objectToAdd));
  }
  static link(context) {
...
Context.add(new Context("global", false, 9999))
...
writeClasses([
  'group/TARGET/GROUPS/GR_BL/GR_BR/GR_TL/GR_TR/TRUE_ID/FALSE_ID/ANIMATION_GID/TARGET_POS/EXTRA_ID/EXTRA_ID_2/FOLLOW/CENTER/TARGET_DIR_CENTER/SHADER_CENTER_ID/GROUP_ID_1/GROUP_ID_2',
  'color/TARGET/TARGET_COLOR/COLOR/COLOR_2/SHADER_TINT_CHANNEL/COPIED_COLOR_ID',
...
let obj_to_levelstring = (l) => {
  ...
  for (var d_ in l) {
    ...
    } else if (explicit[d_] && /*val.value*/val.hasOwnProperty('value')) {
    ...
    //log(key)
    let valid = key !== undefined && val !== undefined
    for (let kov = 0; kov < 2; ++kov) {
      let string = `${kov ? key : val}`
      string.split(".").forEach(n => {
        if (isNaN(+n))
          valid = false
      })
    }
    if (!valid && key != 31/*TEXT*/) {
      log(`expected number, got\nkey ${key}\nval ${val}\nvalJson ${JSON.stringify(val)}\nobj ${JSON.stringify(l)}`) //ñññ
      while(1);
    }
    res += `${key},${val},`;
  }
...
let prep_lvl = () => {
  ...
  for (let i in Context.list) {
    ...
    for (let x in Context.findByName(i).objects) {
      let a = Context.findByName(i).objects[x]
      //console.log(a)
      let r = obj_to_levelstring(a);
      resulting += r;
    }
  }
...
String.prototype.to_obj = function () {
  let or = object({
    OBJ_ID: 914,
    TEXT: btoa(Buffer.from(this, 'utf-8').toString('ascii'))
  });
  return or;
};
